...
export const GET: APIRoute = async ({ request }) => {
  try {
    ...
    if (q) {
      values.push(`%${q}%`);
      filters.push(`(e.title ilike $${values.length} or e.description ilike $${values.length} or e.organizer ilike $${values.length})`);
    }
    if (city) {
      values.push(city);
    ...
    const result = await query(
      `select
         ...
         e.city, e.venue, e.online_url, e.organizer, e.speakers, e.price,
    ...
    if (url.searchParams.get("include_raw") === "1" || rows.length === 0) {
      ...
      if (city) {
        ...
        const cityEn = city === "成都" ? "Chengdu" : "";
        if (cityEn) rawValues.push(`%${cityEn}%`);
        const cityEnIndex = cityEn ? rawValues.length : 0;
        rawFilters.push(`r.city = $${cityIndex}`);
        rawFilters.push(cityEn
          ? `(r.raw_title ilike $${cityNeedleIndex} or r.raw_title ilike $${cityEnIndex})`
          : `r.raw_title ilike $${cityNeedleIndex}`);
      }
